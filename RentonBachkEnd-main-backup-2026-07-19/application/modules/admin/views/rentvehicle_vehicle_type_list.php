				...
				<div class="box">
					...
					<div class="box-body">
						<div class="form-group">
							<button id="add" class="btn btn-sm btn-primary">Tambah Jenis Kendaraan</button>
						</div>
...
<div class="modal fade" id="delete-modal">
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<div class="modal-body">
				Apa kamu yakin ingin menghapus jenis kendaraan ini?
			</div>
			<div class="modal-footer">
				<button type="button" class="btn btn-default" data-dismiss="modal">Batal</button>
...
<script>
	...
	$(document).ready(function(){
		table = $('#list').DataTable({
			...
			"ajax": {
				method : 'GET',
				url : base_url+"admin/rentVehicle/get_list_vehicle_type"
			},
			'columns': [
			{ data: 'id' },
			{ data: 'icon' },
		...
		$('#add').on('click', function (event) {
			$('#input_title').html("Tambah Jenis Kendaraan");
			is_edit = false;
			$('#input')[0].reset();
		...
		$('#list').on('click','.btn-edit', function() {
			$('#input_title').html("Ubah Jenis Kendaraan");
			...
			$.ajax({
				method: 'post',
				url: base_url+"admin/rentVehicle/get_vehicle_type/"+current_id,
				cache: false,
				dataType: 'json',
		...
		$('#post').click(function() {
			...
			$.ajax({
				method: 'post',
				url: base_url+"admin/rentVehicle/post_vehicle_type/"+(is_edit?current_id:""),
				cache: false,
		...
		$('#delete-item').click(function() {
			$.ajax({
				method: 'post',
				url: base_url+"admin/rentVehicle/delete_vehicle_type/"+current_id,
				cache: false,
				dataType: 'json',
				success: function(data){
					if(data.status)

...
import com.nusatim.sapiriku.presentation.feature.register.customer.fragment.RegisterCustomerStepOneFragment
import com.nusatim.sapiriku.presentation.feature.register.customer.fragment.RegisterCustomerStepTwoFragment
...
class RegisterCustomerActivity : AppCompatActivity() {
    ...
    private fun setupViewPager() {
        adapter = RegisterFormPagerAdapter(this).apply {
            addFragment(RegisterCustomerStepOneFragment())
            addFragment(RegisterCustomerStepTwoFragment())
        }
        ...
        binding.vpRegister.registerOnPageChangeCallback(object : ViewPager2.OnPageChangeCallback() {
            override fun onPageSelected(position: Int) {
                ...
                binding.progressRegister.progressDrawable.setColorFilterCompat(
                    ContextCompat.getColor(this@RegisterCustomerActivity, colorBarByValue(position + 1, adapter.itemCount)),
            ...
            repeatOnLifecycle(Lifecycle.State.STARTED) {
                viewModel.registerState.collect { state ->
                    ...
                    when (state) {
                        ...
                        is UiState.Error -> Toast.makeText(this@RegisterCustomerActivity, state.message, Toast.LENGTH_SHORT).show()
                        else -> Unit
                    }
                }
            }

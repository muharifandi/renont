  ...
  <div class="modal-dialog">
	<div class="modal-content">
	  <div class="modal-header">
		...
		<h4 class="modal-title">Verifikasi Topup</h4>
	  </div>
	  <div class="modal-body">
		<div class="form-group">
			...
			<div class="form-group">
				<center>
					<b><span>Nominal : </span></b><span style="color:red;" id="value"></span><br>
					<b><span>Nominal dengan kode : </span></b><span style="color:red;font-size:24px;" id="value_with_code"></span><br>
					<a target="" id="link_img_proof" href="#"><img width="256" id="img_proof" src="" alt="foto bukti tidak terlampir" class="img-thumbnail"></a><br>
				</center>
			</div>
			<label>Status</label>
			<select id="status_select" class="form-control" value="1">
				<?php 
					foreach($list_topup_status as $val)
					{ ?>
...
<script>
	...
	$(document).ready(function(){
		table = $('#list').DataTable({
			...
			"ajax": {
				method : 'GET',
				url : base_url+"admin/customer/get_list_topup"
			},
			'columns': [
				{ data: 'id' },
				{ data: 'fullname' },
			...
			"columnDefs": [
				...
				},
				{
					render : function ( data, type, row ) {
						...
						if(row['status'] == '2')
						{
							return 	"<a href='#' class='btn btn-primary btn-sm' data-target='#verification-modal' data-toggle='modal'"+
							" data-id="+row['id']+
							...
							" data-value-with-code="+encodeURIComponent(row['value_with_code'])+
							" data-img-proof="+encodeURIComponent(row['img_proof'])+
		...
		$('#verification-modal').on('show.bs.modal', function (event) {
			...
			var value_with_code = decodeURIComponent($(event.relatedTarget).data('value-with-code'));
			var img_proof = decodeURIComponent($(event.relatedTarget).data('img-proof'));
			...
			$('#value').html(value);
			$('#value_with_code').html(value_with_code);
		
			if(img_proof !='null')
			{
				$('#img_proof').attr('src', base_url+"data/customers/topup/"+img_proof);
				$('#link_img_proof').attr('href', base_url+"data/customers/topup/"+img_proof);
				$('#link_img_proof').attr('target','blank_');
			}else
			{
				$('#img_proof').attr('src', "");
				$('#link_img_proof').attr('href', "#");
				$('#link_img_proof').attr('target','');
		...
		$('#verification').click(function() {
			...
			$.ajax({
				method: 'post',
				url: base_url+"admin/customer/verification_topup/"+current_id,
				cache: false,
				data :{

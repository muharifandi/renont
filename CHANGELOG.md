...
### [2026-07-23 20:40] Migrasi FCM push notification: Legacy HTTP API (mati) → HTTP v1 (OAuth2)
- **Kenapa:** Google mematikan FCM/GCM Legacy HTTP API pada Juni 2024 — `application/libraries/Fcm.php` masih memanggil endpoint lama (`fcm.googleapis.com/fcm/send` dengan server key sederhana) tanpa error yang terlihat di aplikasi (17 titik panggilan di 11 controller, semuanya fire-and-forget, tidak ada yang cek hasil `send()`), tapi push notification sudah lama sepenuhnya tidak terkirim.
- **Implementasi:** `Fcm.php` ditulis ulang total memakai FCM HTTP v1 — autentikasi lewat OAuth2 service account: JWT ditandatangani sendiri (`openssl_sign`, RS256) dari file JSON service account, ditukar ke access token lewat `oauth2.googleapis.com/token` (token di-cache ke file sementara, dipakai ulang sampai ~1 menit sebelum kedaluwarsa, supaya tidak tukar token di setiap kirim notifikasi), lalu request ke `fcm.googleapis.com/v1/projects/{project_id}/messages:send`. Murni curl + openssl, tanpa Composer/library resmi Google (proyek ini tidak pakai Composer).
- **Nama & signature method publik dipertahankan 100% sama** (`setData`, `setNotification`, `addRecepient`, `setRecepients`, `setGroup`, `setTtl`, `send()`) — dicek dulu semua 17 call site memang tidak pernah membaca `->status`/`->messagesStatuses`/return value `send()`, jadi rewrite ini **tidak menyentuh satu pun dari 11 file controller yang memanggilnya**.
- **Perbedaan perilaku yang disengaja**: constructor versi lama memanggil `show_error()` (mematikan seluruh request) kalau config API key kosong. Versi baru sengaja TIDAK melakukan itu — kalau service account belum ada/tidak valid, `send()` cuma log error & return `false`, supaya alur inti (booking, chat, topup, dll — semua fitur yang notifikasinya cuma efek samping) tidak ikut mati gara-gara FCM belum dikonfigurasi.
- **Payload**: field `android_channel_id`/`sound` yang dulu "diselundupkan" sebagai key custom di dalam `notification` (bukan field asli FCM) sekarang dipindah dengan benar ke `android.notification.channel_id`/`sound`. Semua value di `data` dikoersi ke string (wajib di v1). `title`/`text`/`body` dinormalisasi (kode lama pakai `text` di beberapa tempat dan `body` di tempat lain — sekarang keduanya didukung sebagai fallback ke `notification.body`).
- **Keterbatasan**: FCM v1 tidak punya endpoint multicast seperti legacy (`registration_ids` array dalam 1 request) — sekarang loop 1 HTTP request per token penerima. Cukup untuk skala saat ini; broadcast ke seluruh partner/customer (`News::send_notification`) bisa jadi lambat kalau daftar penerimanya sangat besar suatu saat nanti — topic messaging adalah perbaikan lanjutan yang belum dikerjakan.
- **Config**: `application/config/{development,testing,production}/fcm.php` — `fcm_api_key`/`fcm_api_key_android`/`fcm_api_send_address` diganti `fcm_service_account_path` (default: `RentonBachkEnd-main/.fcm-service-account.json`, override lewat `FCM_SERVICE_ACCOUNT_PATH` di `.env.{environment}`). `FCM_API_KEY` (server key legacy, sudah mati) dihapus dari `.env.example`, dikomentari di ketiga `.env.{environment}` nyata. `.gitignore` ditambah `**/*fcm-service-account*.json` — file ini berisi RSA private key, tidak boleh pernah ter-commit.
- **Diverifikasi tanpa kredensial asli** (karena OAuth2 service account harus dibuat manual oleh pemilik project Firebase): dites dengan RSA keypair & service account palsu — struktur JWT benar & signature-nya valid (diverifikasi ulang pakai public key pasangannya), transformasi payload sesuai ekspektasi untuk beberapa bentuk payload nyata dari codebase, dan percobaan tukar token ke Google dengan kredensial palsu gagal dengan `invalid_grant: account not found` (bukan error format) — bukti JWT-nya sudah valid diterima Google, tinggal butuh service account yang asli.
- **⚠️ Belum aktif sampai user menyediakan 1 file**: kredensial OAuth2 (`.fcm-service-account.json`) harus diunduh manual dari Firebase Console oleh pemilik project (Claude tidak bisa mengaksesnya) — lihat instruksi lengkap di TODO.md §🔔.

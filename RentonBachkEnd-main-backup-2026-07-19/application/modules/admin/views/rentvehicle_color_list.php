				...
				<div class="box">
					...
					<div class="box-body">
						<div class="form-group">
							<button id="add" class="btn btn-sm btn-primary">Tambah Warna</button>
						</div>
	...
	<div class="modal-dialog">
		<div class="modal-content">
			<div class="modal-header">
				...
				<h4 id="input_title" class="modal-title">Tambah Warna</h4>
				</div>
...
<div class="modal fade" id="delete-modal">
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<div class="modal-body">
				Apa kamu yakin ingin menghapus warna ini?
			</div>
			<div class="modal-footer">
				<button type="button" class="btn btn-default" data-dismiss="modal">Batal</button>
...
<script>
	...
	$(document).ready(function(){
		table = $('#list').DataTable({
			...
			"ajax": {
				method : 'GET',
				url : base_url+"admin/rentVehicle/get_list_color"
			},
			'columns': [
			{ data: 'id' },
			{ data: 'value' },
			...
			},
			{
				render : function ( data, type, row ) {
					...
					if(row['value'] != null)
					{
						return "<div class='img-thumbnail' style='width:24px;height:24px;background-color:"+row['value']+";'/>";
					}else
		...
		$('#add').on('click', function (event) {
			$('#input_title').html("Tambah Warna");
			is_edit = false;
			$('#input')[0].reset();
			$('input[name ="value"]').val('#000000');
			$('.my-colorpicker').colorpicker('destroy');
			$('.my-colorpicker').colorpicker({'color':'#000000'});
			$('#add-modal').modal('show');
		});
		
		$('#list').on('click','.btn-edit', function() {
			$('#input_title').html("Ubah Warna");
			...
			$.ajax({
				method: 'post',
				url: base_url+"admin/rentVehicle/get_color/"+current_id,
				cache: false,
				dataType: 'json',
				success: function(data){
					if(data.status)
					{
						...
						$('.my-colorpicker').colorpicker('destroy');
						$('.my-colorpicker').colorpicker({'color':data.data.value});
		...
		$('#post').click(function() {
			...
			$.ajax({
				method: 'post',
				url: base_url+"admin/rentVehicle/post_color/"+(is_edit?current_id:""),
				cache: false,
		...
		$('#delete-item').click(function() {
			$.ajax({
				method: 'post',
				url: base_url+"admin/rentVehicle/delete_color/"+current_id,
				cache: false,
				dataType: 'json',
				success: function(data){
					if(data.status)
